...
function charity_ticker_animate(animate_charity_ticker_start, animate_charity_ticker_end ){

	console.log("Running Ticker Animation: " + animate_charity_ticker_start + " , " + animate_charity_ticker_end + ' , at the speed of ' + charity_animation_speed);
	new CountUp(
		"charity_total",
		animate_charity_ticker_start,
		animate_charity_ticker_end,
		0,
		charity_animation_speed,
		{
			useEasing : true,
			useGrouping : true,
			separator : ',',
...
function get_recent_charity(){
	...
	if( charity_ajax_request_count > charity_ajax_request_max ){
		return null;
	}

	jQuery.ajax({
		url: ticker_charity_box_url,
		method: "GET",
		dataType: "JSON",
		data: {
			timestamp: Date.now()
		...
		if( charity_value_last != response.charity_total){
			...
			if( response.charity_prev == 0){
				...
				new CountUp(
					"charity_total",
					0,
					response.charity_prev,
					0,
					charity_initial_animation_speed,
					{
						useEasing : true,
						useGrouping : true,
						separator : ',',
